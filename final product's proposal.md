...
Users can manage (CRUD) plants with details like name, species, watering frequency, location (inside/outside), and notes.
  ...
  |--------------------------------------|----------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
  | User submits scientific_name         | Saves the result of the frontend's "save" POST request | After scientific name typed, makes request to Gemini to auto-fill plant details into the frontend form. Make another request to Google image search, scrape first image result URL from response. There's a persistent banner warning people of AI usage and encouraging diligence at the bottom of the page, "Plant Pal uses AI to make adding plants faster, but we recommend checking the generated info for accuracy". Site shouldn't reload upon AI generation and image fetching. |
